...
//    final static String DESTINATION_FOLDER = '/Volumes/EasyStore'
    ...
    void makeCSV() {
        String output = ''
        execData.toOrganize.each { FileData it ->
            String line = "${it.filename},${it.fullPath},${it.fileSize},${it.originalDate}\n"
            output += line
        }
        File executionFile = new File("$DESTINATION_FOLDER/execution.csv")
        executionFile.text = output
    }
    ...
    void applyDestinationFilePath(FileData fileData) {
        ...
        File destinationFile = new File("$filePath/$fileData.filename.$fileData.fileExtension")
        while (destinationFile.exists()) {
            collisionCount++
            fileData.duplicate = true
            execData.duplicates++
            destinationFile = new File("$filePath/$fileData.filename-$collisionCount.$fileData.fileExtension")
        }
